use std::sync::Arc;
use crate::render::vulkan::core::{Core, AshRaw, Image};
use ash::vk;
...
impl Drop for ImageView {
    fn drop(&mut self) {
        unsafe {
            self.core.device.destroy_image_view(self.image_view,Option::None)
        }
    }
}
...
impl ImageViewBuilder {
    ...
    pub fn build(self) -> Result<ImageView,vk::Result> {
        let image_view = unsafe {
            self.core.device.create_image_view(&self.info,Option::None)
        }?;
        Ok(ImageView{
            core : self.core,
            image_view
