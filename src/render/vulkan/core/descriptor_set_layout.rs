...
impl Drop for DescriptorSetLayout {
    fn drop(&mut self) {
        unsafe {
            self.core.device.destroy_descriptor_set_layout(self.descriptor_layout,Option::None)
        }
    }
}
...
impl DescriptorSetLayoutBuilder {
    pub fn binding(mut self,
                   ...
                   stage : vk::ShaderStageFlags) -> Self{
    ...
    pub(in crate::render) fn build(self,core : Arc<Core>) -> Result<DescriptorSetLayout,vk::Result> {
        let info = vk::DescriptorSetLayoutCreateInfo::builder()
            .bindings(&self.bindings);
        let layout = unsafe {
            core.device.create_descriptor_set_layout(&info,Option::None)
        }?;
