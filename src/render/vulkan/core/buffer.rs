use crate::render::vulkan::core::{AshRaw, CommandBuffers, Core, DeviceMemory};
use ash::vk;
use std::sync::Arc;
...
impl Buffer {
    ...
    pub fn memory_requirements(&self) -> vk::MemoryRequirements {
        unsafe { self.core.device.get_buffer_memory_requirements(self.buffer) }
    }
    ...
    pub fn copy_from_buffer(&mut self, source: &Buffer) -> Result<(), vk::Result> {
        debug_assert!(
            self.has_memory() | source.has_memory(),
        ...
        unsafe {
            ...
            self.core
                .device
                .cmd_copy_buffer(command_buffer, source.buffer, self.buffer, &copy);
...
impl Drop for Buffer {
    fn drop(&mut self) {
        unsafe { self.core.device.destroy_buffer(self.buffer, Option::None) }
    }
}
...
impl BufferBuilder {
    ...
    pub(in crate::render) fn build(self, core: Arc<Core>) -> Result<Buffer, vk::Result> {
        ...
        let buffer = unsafe { core.device.create_buffer(&info, Option::None) }?;
        Ok(Buffer {
            core,
            buffer,
            size: self.size,
