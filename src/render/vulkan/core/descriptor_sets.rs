...
use crate::render::vulkan::core::{AshRaw, Core, ImageView};
use ash::vk;
use std::sync::Arc;
...
impl DescriptorSets {
    ...
    pub(in crate::render) fn allocate(
        ...
        let info = vk::DescriptorSetAllocateInfo::builder()
            ...
            .set_layouts(&layouts);
        let descriptor_sets = unsafe { core.device.allocate_descriptor_sets(&info) }?;
        Ok(DescriptorSets {
            core,
            descriptor_sets,
        })
...
impl Drop for DescriptorSets {
    fn drop(&mut self) {
        unsafe {
            self.core
                .device
                .free_descriptor_sets(self.core.descriptor_pool, &self.descriptor_sets)
                .unwrap();
        }
    }
}
