use crate::render::vulkan::core::{Core, AshRaw, RenderPass, ImageView};
use std::sync::Arc;
use ash::vk;
...
impl Drop for Framebuffer {
    fn drop(&mut self) {
        unsafe {
            self.core.device.destroy_framebuffer(self.framebuffer,Option::None)
        }
    }
}
...
impl FramebufferBuilder {
    ...
    pub(in crate::render) fn build(self,core : Arc<Core>) -> Result<Framebuffer,vk::Result> {
        ...
        let framebuffer = unsafe {
            core.device.create_framebuffer(&info,Option::None)
        }?;
        Ok(Framebuffer{
            core,
            framebuffer
