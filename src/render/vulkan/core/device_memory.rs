...
impl DeviceMemory {
    pub fn copy_from_slice<T : Sized + Copy>(&mut self,slice : &[T]) -> Result<(),vk::Result> {
        ...
        let mut align = unsafe {
            util::Align::new(ptr,std::mem::align_of::<T>() as _,self.reqs.size)
        };
    ...
    fn find_memory(memory_properties : &vk::PhysicalDeviceMemoryProperties,
                   reqs : vk::MemoryRequirements,
                   flags : vk::MemoryPropertyFlags) -> Option<usize> {
    ...
    pub(in crate::render) fn allocate(core : Arc<Core>,reqs : vk::MemoryRequirements ,flags : vk::MemoryPropertyFlags)
        ...
        let memory = unsafe {
            core.device.allocate_memory(&info,Option::None)
        }?;
        Ok(DeviceMemory{
            core,
            reqs,
...
impl Drop for DeviceMemory {
    fn drop(&mut self) {
        unsafe {
            self.core.device.free_memory(self.device_memory,Option::None);
        }
    }
}
