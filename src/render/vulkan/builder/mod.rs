...
use crate::render::vulkan::{PresentMode, Vulkan};
...
impl VulkanBuilder {
    ...
    pub fn build(self, window: &Window) -> Result<Vulkan, BuildVulkanError> {
        ...
        // create instances
        let instance = unsafe { entry.create_instance(&instance_info, Option::None) }?;
        ...
        let device_layer_ptrs = device_layers
            .iter()
            .map(|str| str.as_ptr())
            .collect::<Vec<_>>();
        ...
        let device =
            unsafe { instance.create_device(physical_device, &device_info, Option::None) }?;

        let graphics_queue = unsafe { device.get_device_queue(queue_family_index, 0) };
        let transfer_queue = unsafe { device.get_device_queue(queue_family_index, 1) };

        // create command pool
        let command_pool_info = vk::CommandPoolCreateInfo::builder()
            .queue_family_index(queue_family_index)
            .flags(vk::CommandPoolCreateFlags::RESET_COMMAND_BUFFER);
        let command_pool = unsafe { device.create_command_pool(&command_pool_info, Option::None) }?;
        ...
        let format = surface_formats
            .iter()
            .cloned()
            .find(|format| format.format == self.format && format.color_space == self.color_space)
        ...
        let swapchain =
            unsafe { swapchain_manager.create_swapchain(&swapchain_info, Option::None) }?;

        let swapchain_images = unsafe { swapchain_manager.get_swapchain_images(swapchain) }?;
        ...
        for image in swapchain_images {
            ...
            let view = unsafe { device.create_image_view(&image_view_info, Option::None) }?;
        ...
        let descriptor_pool =
            unsafe { device.create_descriptor_pool(&descriptor_pool_info, Option::None) }?;
...
unsafe extern "system" fn debug_callback(
    ...
    #[allow(unused)] message_types: vk::DebugUtilsMessageTypeFlagsEXT,
